...
const Details_present = () => {
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        try {
                            axios.delete(`http://localhost:61713/api/timeshit/${id}`)
                                .then((res) => {
                                    if (res.status === 200) {
        ...
        },
        {
            ...
            cell: (row) =>
                <div className='d-flex align-items-center'>
                    <Link className='btn btn-primary btn-sm ' to={`/Admin_View/present/${row.id}`} >View</Link>
    ...
    return (<>
        ...
        <Link className='btn btn-primary mt-2 ' style={{position:"relative",float:"right"}} to={`/Admin`}>Back To Admin</Link>
...
export default Details_present;
